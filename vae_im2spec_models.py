...
class vae_im2spec(nn.Module):
    ...
    def __init__(self,
        ...
        self.enc_fc_mu = nn.Linear(self.e_filt * self.n * self.m, latent_dim)
        self.enc_fc_logvar = nn.Linear(self.e_filt * self.n * self.m, latent_dim)
    ...
    def encoder(self, features: torch.Tensor):
        """
    ...
    def decoder(self, encoded: torch.Tensor):
        """
    ...
    def predict(self, x: torch.Tensor):
        ...
        with torch.no_grad():
            pred_spectra, _, _  = self.forward(x)
            
            return pred_spectra
        


class vae_im2spec_2(nn.Module):
    """
    Encoder (2D) - decoder (1D) type model for generating spectra from image
    """
    def __init__(self,
...
class vae_im2spec_4(nn.Module):
    ...
    def __init__(self,
        ...
        self.enc_fc_mu = nn.Linear(self.e_filt * self.n//2 * self.m//2, latent_dim)
        self.enc_fc_logvar = nn.Linear(self.e_filt * self.n//2 * self.m//2, latent_dim)
...
#         else:
#             self.models = [model(input_dim, out_put_dim, latent_dim = np.random.randint(8, 32)) for model in models]
